...
extension UIImage {
  ...
  func scaledData(with size: CGSize, byteCount: Int, isQuantized: Bool) -> Data? {
    ...
    print("DEBUG: scaledBytes - ", scaledBytes.count)
    var scaledFloats2: Array<Float> = Array(repeating: 0, count: scaledBytes.count)
    for x in 0..<224 {
        for y in 0..<224 {
            var current_index = self.coordinateToIndex(x: x, y: y, z: 0)
            scaledFloats2[current_index] = Float32(scaledBytes[current_index])/255.0 - Constant.imageMeanR
            
            current_index = self.coordinateToIndex(x: x, y: y, z: 1)
            scaledFloats2[current_index] = Float32(scaledBytes[current_index])/255.0 - Constant.imageMeanG
            
            current_index = self.coordinateToIndex(x: x, y: y, z: 2)
            scaledFloats2[current_index] = Float32(scaledBytes[current_index])/255.0 - Constant.imageMeanB
        }
    }
    // let scaledFloats = scaledBytes.map { (Float32($0) - Constant.imageMean) / Constant.imageStd }
    // print("DEBUG: scaledFloats ", scaledFloats.count, scaledFloats[0], type(of: scaledFloats))
    print("DEBUG: scaledFloats2 ", scaledFloats2.count, scaledFloats2[0], type(of: scaledFloats2))
    return Data(copyingBufferOf: scaledFloats2)
  }
...
private enum Constant {
    ...
    static let imageMeanR: Float32 = 0.517
    static let imageMeanG: Float32 = 0.416
    static let imageMeanB: Float32 = 0.363
}
